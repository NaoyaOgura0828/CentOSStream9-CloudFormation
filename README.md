# CentOSStream9-CloudFormation

`CentOSStream9-CloudFormation`は`AWS CloudFormation`を利用し、`CentOS Stream 9 (Official)`の実行環境を構築する。
<br>
...
2021-11-21現在の適用`CentOS Stream 9 (Official) AMI ID`
- ami-054a46b335b2fe0b8
...
2. キーペア名を登録する。
    <br>
    `\CentOSStream9\ami-build`及び`CentOSStream9\ec2`内の`{環境名}-parameters.json`へキーペア名を設定する。
...
6. `AMI ID`を設定する。
    <br>
    `AWSコンソール`へアクセスし、`\EC2\AMI\CentOSStream9-{環境名}-ec2-ami`の`AMI ID`をコピーする。
    <br>
    `\CentOSStream9\ec2\{環境名}-parameters.json`内の`AMIID`へペーストする。
    <br>
...
#####################################
# /CentOSStream9/ec2/{EnvType}-parameters.json
...
- EC2の構成は`\CentOSStream9\ec2\{環境名}-parameters.json`内で任意に行うことが出来る。
- **デフォルトユーザー名は2021-11-21現在不明である。**
